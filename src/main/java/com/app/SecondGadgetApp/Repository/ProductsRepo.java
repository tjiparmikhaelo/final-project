...
{
    @Query(value = "select * from products p where not p.product_status = 'archive' and not p.product_status = 'sold' order by created_at desc limit 8", nativeQuery = true)
    ...
    @Modifying
    @Query(value = "select * from products p where category_id = ?1 and not p.product_status = 'archive' and not p.product_status = 'sold' order by created_at desc limit 4", nativeQuery = true)
